...
contract Transaction {
	...
	event HelloValueLogger(uint helloValue);
	event HelloSenderLogger(address sender);
	...
	constructor(address HelloAddress) public {
	    owner = msg.sender;
		hello = Hello(HelloAddress);
	 ...
	 */
	function setValueToHelloContract(uint _value) external payable {
		hello.t21(_value);
	 ...
	 */
	function getValueFromHelloContract() public returns (uint) {
	    uint value = hello.t22();
		emit HelloValueLogger(value);
		return value;
	}
	 ...
	 */
	function getSenderFromHelloContract() public returns (address) {
        address sender = hello.t23(); //实验证明，返回的是本合约地址	
		emit HelloSenderLogger(sender);
		return sender;
	}
	 ...
	 */
	function getValueFromHelloContractWithoutABI() public returns (uint) {
		//这行演示调用外部合约中一个不存在的函数也会返回成功
        //if(address(hello).call(bytes4(keccak256("t203()"))))
		//    emit SenderLogger(msg.sender);

		//call()、send()、delegatecall()、callcode()
		//函数在失败时不会抛出异常，而是会返回false
		if(!address(hello).call(bytes4(keccak256("t23"))))
			revert();
	 ...
	 */
	function sendEtherToHelloContract() public {
        //如果异常会回滚，会导致当前合约抛出异常并停止往下执行. 不能设置gas(默认2300)	
        address(hello).transfer(100000000); 
        
		//如果失败会返回false，不回滚, 不会导致本合约异常终止.不能设置gas（默认2300）
		require(address(hello).send(200000000)); 

		//这种方式具有被调用合约反调用的风险。可以设置gas
		if(!address(hello).call.value(300000000).gas(6000)())
		   revert();	
